function [A b] = getBetaParametersProblemIndependentSkin(k, L, qtotal, wb, rhob, cb, To, qo)
%
...
%
% 
% File:   getBetaParametersProblemIndependentSkin.m
...
b = zeros(2*size(k,2) - 1, 1);
A = sparse(2*size(k,2) - 1, 2*size(k,2)); % zeros(2*size(k,2), 2*size(k,2));
