function predictionsSet = solveSetKM(set_i, heat_power_in = NA, Ta_pen_in = NA, ...
...
%
% Usage:
% predictionsSet = solveSetKM(set_i, heat_power_in, Ta_pen_in, Ta_brooder_in, ...
...
filename = [folder_sets "/KM/S" num2str(set_i) ".mat"];
if (loadSets && exist(filename) == 2)
    % if the specific load file of the KM model does not exist, we try to load
  ...
  if ( exist(filename) != 2 )
    error(["KM: The file with the data for set " num2str(set_i) " was not found"]);
  end
  ...
  "d", "epsilong", "dg", "z", "Lt", "TMR_m", "h_m", "h_m_skin", "omega", "phi", ...
...
for position_i = 1:length(positions)
  ...
  % Kowalski and Mitchel (1979) for long-wave radiation:
  [T_int, q_int, beta, obj, info, iter, nf, lambda, eqConst] = ...
      solveAnalyticalModelKM(Ta_brooder, k, L, qo, d, z, Lt, h_m, h_m_skin, omega, phi, ua, NTissueLayers, ...
  ...
  elseif (info == 104)
    ...
    if (obj > objTol || max(abs(eqConst)) > eqTol)
      ...
      disp(["KM: #set: " num2str(set_i) " #sample: " num2str(sample_i) ...
  ...
  predictionsSet(position_i, 2) = q_int(NTissueLayers + 1); % q_skin_ana;
  predictionsSet(position_i, 3) = Th_ana;
...
if (loadMeasuredData)
  ...
  disp([" KM: #Set: " num2str(set_i) "; Time: " num2str(totalTime, "%2.4f") ...
