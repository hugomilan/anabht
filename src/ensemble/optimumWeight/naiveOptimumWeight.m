function [MSE_optimumWeights setsOptimumWeights naiveOptimumWeights ...
          LL_optimumWeights Obj_optimumWeights] = ...
                        naiveOptimumWeight(predictions, ...
...
% that improve MSE but DOES NOT remove the sets that end up with weight 0. These 
% sets are kept for possible finding minimum objective values
%
%
%
% Usage:
% [MSE_optimumWeights setsOptimumWeights naiveOptimumWeights] = ...
%                         naiveOptimumWeight(predictions, ...
%                                             samplePositions, testingPositions, ...
...
% sets: sets ID to do the greedy search
...
% setsOptimumWeights: Vector that contains the position of the sets selected during the
%                     greedy search.
% naiveOptimumWeights: cell that contains the weights for each greedy search.
...
naiveOptimumWeights = cell(1,totalNumberOfSets);
...
setsOptimumWeights{ii} = setToTest;
naiveOptimumWeights{ii} = weightsToTest;

disp([msg "Naive search with optimum weight (lambda = " num2str( lambda, "%2.4f" ) ") " num2str(ii) ...
                  ...
                  "; non-zero weights: " num2str( sum(naiveOptimumWeights{ii} > 0) )]);
                     
for ii = (numberOfInitialSets + 1):totalNumberOfSets
    setToTest = [setsOptimumWeights{ii - 1}; sets(ii)];
    [MSEToTest weightsToTest LLToTest ObjToTest] = optimumWeightsMSE(predictions, ...
              setToTest, samplePositions, testingPositions, ...
              solveFor, data, [naiveOptimumWeights{ii - 1}; 0], lambda);
