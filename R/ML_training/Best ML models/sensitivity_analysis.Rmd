...
# Legend
legend("topleft", c(expression(paste('T'["a"], " (LM)", sep="")), expression(paste('T'["g"], " (LM)", sep=""))), col = c(Ta_brooder.color, Tg_brooder.color), lty = c(1, 1), lwd = 2,  pch = c(NA, NA), pt.cex = symbol.size, cex = 1.6, horiz = T, x.intersp = 0.3, y.intersp = 0.5)
```
...
################## Ploting for all (Ta_brooder, Tg_brooder, and Tr)
par(mar = c(5, 4, 4.5, 2) + 0.1)
plot(hours.plot, mean.dataset$Tr, xlim = c(1,24), ylim=c(36.8, 38.8), xaxt = "n", col = Ta_brooder.color, pch = measured.symbol, ylab = expression(paste("Temperature ("^{o},"C)", sep = "")), xlab = "Time (hour)", mgp=c(2.3,0.7,0), cex=symbol.size, cex.axis = 1.4, cex.lab = 1.6)
# including ticks
# axis(2, at = c(18:30), labels = FALSE)
# power_source = 0
axis(1, at = c(4, 5, 6, 7) - 3, labels = c(4, 5, 6, 7), cex.axis = 1.4)
...
# writing power
par(xpd = TRUE)
text(x = 2.5, y = 39, "0 W", cex = 1.6)
text(x = 2.5+5, y = 39, "60 W", cex = 1.6)
text(x = 2.5+10, y = 39, "100 W", cex = 1.6)
text(x = 2.5+15, y = 39, "160 W", cex = 1.6)
text(x = 2.5+20, y = 39, "200 W", cex = 1.6)
par(xpd = FALSE, mar = c(5, 4, 4, 2) + 0.1)

# Legend
legend("bottomright", c(expression(paste('T'["r"], " (LM)", sep=""))), col = c(Tr.color), lty = c(1), lwd = 2,  pch = c(NA), pt.cex = symbol.size, cex = 1.6, horiz = T, x.intersp = 0.3, y.intersp = 0.5)
```
...
# export as 312 x 479
par(mar = c(2, 4, 0.5, 1))
plot(plot.positions, c(mean_Ta_brooder_percentage_error, mean_Tg_brooder_percentage_error, mean_Tr_percentage_error), xlim = c(0.9,3.1), ylim=c(0.5, 2), xaxt = "n", col = plot.colors, pch = plot.symbols, ylab = expression(paste("Percentage error (%)", sep = "")), xlab = "", mgp=c(1.5,0.4,0), cex=symbol.size, cex.axis = 1.4, cex.lab = 1.6)
